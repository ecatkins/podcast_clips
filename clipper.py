...
class Clipper:
    ...
    def best_sliding_window(self, word_embeddings, whole_document_embedding, text_list): 
        ...
        for i in range(len(word_embeddings) - window_size):
            similarity = 1 - cosine(whole_document_embedding, np.mean(word_embeddings[i:i+window_size], axis=0))
            # print(similarity)
        ...
        print(' '.join(text_list[max_similarity_index:max_similarity_index+window_size]))
    ...
    def cut_audio(self, result):
        ...
        sf.write(self.save_loc, data[int(start_time*samplerate):int(end_time*samplerate)], samplerate)
    ...
    def run(self):
        ...
        window_start, window_end = self.best_sliding_window(word_embeddings, whole_document_embedding, text_list)
        ...
        text = " ".join([item['alternatives'][0]['content'] for item in item_list[window_start:window_end]])


        result = {'window_start_token': window_start, 'window_end_token': window_end, 'text': text, 'item_list':item_list}
        return result
